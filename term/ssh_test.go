...
func TestSSH(t *testing.T) {
	...
	sTerm.Cflag &= syscall.CS7 | syscall.CS8 | syscall.PARENB | syscall.PARODD // The only term control modes in the SSH standard.
	if sTerm != dTerm {
	...
	sTerm.Cook()
	dTerm.FromSSH(sTerm.ToSSH())
	dTerm.Iflag += syscall.BRKINT // Not in the SSH standard but set for Linux cooked mode.
