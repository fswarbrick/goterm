...
func testraw(tr Termios, f string) error {
	if (tr.Iflag & (syscall.IGNBRK | syscall.BRKINT | syscall.PARMRK | syscall.ISTRIP | syscall.INLCR |
		syscall.IGNCR | syscall.ICRNL | syscall.IXON)) != 0 {
		return fmt.Errorf("%q Raw failed setting c_iflag , got: %d want: 0", f, tr.Iflag)
	}
	if (tr.Oflag & syscall.OPOST) != 0 {
		return fmt.Errorf("%q Raw failed setting Oflag , got: %d want: 0", f, tr.Oflag)
	}
	if (tr.Lflag & (syscall.ECHO | syscall.ECHONL | syscall.ICANON | syscall.ISIG | syscall.IEXTEN)) != 0 {
		return fmt.Errorf("%q Raw failed setting Lflag, got: %d want: 0", f, tr.Lflag)
	}
	if (tr.Cflag & (syscall.PARENB)) != 0 {
		return fmt.Errorf("%q Raw failed setting Cflag , got: %d want: 0", f, tr.Cflag)
	}
	if (tr.Cflag & syscall.CSIZE) != syscall.CS8 {
		return fmt.Errorf("%q Raw failed setting Cflag CS8, got: %d ", f, tr.Cflag)
	}
	if !(tr.Cc[syscall.VMIN] == 1 && tr.Cc[syscall.VTIME] == 0) {
...
func testcook(tr Termios, f string) error {
	if tr.Iflag&(syscall.BRKINT|syscall.IGNPAR|syscall.ISTRIP|syscall.ICRNL|syscall.IXON) !=
		syscall.BRKINT+syscall.IGNPAR+syscall.ISTRIP+syscall.ICRNL+syscall.IXON {
		return fmt.Errorf("%q Cook failed setting Iflag , got: %d want: %d", f, tr.Iflag,
			syscall.BRKINT+syscall.IGNPAR+syscall.ISTRIP+syscall.ICRNL+syscall.IXON)
	}
	if (tr.Oflag & syscall.OPOST) != syscall.OPOST {
		return fmt.Errorf("%q Cook failed setting Oflag , got: %d want: %d", f, tr.Oflag, syscall.OPOST)
	}
	if (tr.Lflag & (syscall.ISIG | syscall.ICANON)) != syscall.ISIG+syscall.ICANON {
		return fmt.Errorf("%q Cook failed setting Lflag , got: %d want: %d", f, tr.Lflag,
